...
AI 学习笔记站点，基于 **VitePress**（Vue 官方文档同款）构建，四个系列共 51 个页面（50 篇图解长文 + 1 个可搜索资源导航页）。
...
目前 **88 个站点、八类**（模型与数据 / 论文与前沿 / 评测与榜单 / 学习与课程 / 资讯与社区 / 工具与框架 / **Agent Skills** / 中文生态）。导航页里对 Agent Skills 只给分层速查表，完整展开放在第 05 篇：四层来源怎么用、怎么判断一个技能值不值得装、装之前的四关安全核验、装完怎么管住数量。
...
| 05 | Skill：去哪找，装之前查什么 | `docs/ai-sites/05-skills.md` |

五篇分类指南各配一张图解，讲的不是"有哪些站"，而是**用法与坑**：模型许可证与量化版本怎么查、论文怎么用日榜建立基线再用引用图追脉络、榜单的五个追问、学习三个阶段的材料顺序，以及 Skill 的四层来源与装之前的四关安全核验。站点的可达性与核实日期记录在数据文件里，详情页只引用结论。
